...
const PostDetailPage = ({ post }: { post: PostDetail }) => {
  ...
  const posts = useSelector(
    (state: IRootState) => state.storedPostsReducer.posts
  );
  ...
  const topicsList: string[] = post.topics
    ? post.topics.edges.map((topic) => topic.node.name)
    : [];
  const topicFilterPosts = posts.filter(
    (eachPost) =>
      eachPost.topics.edges.some((topic) =>
        topicsList.includes(topic.node.name)
      ...
      <div className="container mx-auto px-1 lg:px-4 my-14 flex flex-col lg:flex-row lg:justify-between">
        <div className="w-full lg:w-7/12 lg:mr-3 mb-6 lg:mb-0">
          <Link
            href="/"
            passHref
            className="inline-flex items-center text-blueGray-400 mb-4"
          >
            <ArrowBackIosIcon className="text-sm " />
            {t("homepage.backButton")}
          </Link>
        ...
        <div className="w-full lg:w-5/12">
          <RelatedTopicsSection
            displayPost={post}
            posts={posts}
            topicFilterPosts={topicFilterPosts}
          />
        </div>
      </div>
    </>
