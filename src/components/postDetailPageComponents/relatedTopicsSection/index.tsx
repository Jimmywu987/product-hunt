...
function RelatedTopicsSection({
  ...
  topicFilterPosts: PostDetail[];
}) {
  const { t } = useTranslation("common");
  const loading = useSelector(
    (state: IRootState) => state.loadingStatusReducer.status
  );
  if (loading) {
    return (
    ...
    <div>
      <div className="text-xl mb-3 ml-2 text-blueGray-700">
        {topicFilterPosts && topicFilterPosts.length > 0
          ? t("postDetailPage.relatedPostTitle")
          : t("postDetailPage.otherPostTitle")}
      </div>
      <div className="w-full grid grid-cols-1 gap-6 lg:px-3">
        {topicFilterPosts && topicFilterPosts.length > 0
          ...
          : posts
              .filter((post) => {
                return post.name !== displayPost.name;
              })
              .slice(0, 2)
              .map((post) => <EachPostGrid key={post.slug} post={post} />)}
      </div>
