            ...
            <label className="flex items-center cursor-pointer">
                <div>
                  <Checkbox
                    ...
                    checkedIcon={
                      <CheckBox className="text-theme-red" fontSize="small" />
                    }
                  />
                </div>
  ...
  return (
    <div className="p-1">
        <div className="hidden lg:flex mb-4 text-blueGray-700 text-xl  items-center">
            ...
            {topicFilter.length > 0 && <span className="ml-2 text-sm text-theme-red self-end">{topicFilter.length}</span>}
        </div>
        <div className="h-[580px] scrollbar scrollbar-w scrollbar-thumb-theme-red  p-2 border ">
            {topics.map((topic, index)=>{
                return <EachFilterOption topic={topic} setTopicFilter={setTopicFilter} topicFilter={topicFilter} key={`${index}-${topic}`}/>
            })}
        </div>
    </div>
  )
